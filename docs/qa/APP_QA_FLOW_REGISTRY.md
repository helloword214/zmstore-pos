...
| `delivery-rider-acceptance-path` | `P0` | delivery | rider | accept a manager-approved `CHARGE_RIDER` shortage in the rider variance lane | `docs/guide/CANONICAL_DELIVERY_CASH_AR_FLOW.md`, `docs/guide/RIDER_SHORTAGE_WORKFLOW.md` | canonical coverage is now `scripts/qa/delivery/delivery-rider-acceptance-path-setup.ts`, `scripts/qa/delivery/delivery-rider-acceptance-path-cleanup.ts`, `docs/qa/delivery/delivery-rider-acceptance-path-checklist.md`, `tests/ui/delivery/delivery-rider-acceptance-path-fixture.ts`, and `tests/ui/delivery/delivery-rider-acceptance-path.spec.ts` | `keep` | `scenario-family-ready` | extend delivery coverage with final cashier settlement gating and post-acceptance charge collection or payroll follow-through after the rider acknowledgement lane is stable |
