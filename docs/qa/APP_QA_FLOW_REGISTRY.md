...
| `delivery-manager-shortage-review-charge-path` | `P0` | delivery | store manager | review an open rider shortage variance from the queue and charge the rider so the case moves to rider-acceptance state | `docs/guide/CANONICAL_DELIVERY_CASH_AR_FLOW.md`, `docs/guide/RIDER_SHORTAGE_WORKFLOW.md` | canonical coverage is now `scripts/qa/delivery/delivery-manager-shortage-review-charge-path-setup.ts`, `scripts/qa/delivery/delivery-manager-shortage-review-charge-path-cleanup.ts`, `docs/qa/delivery/delivery-manager-shortage-review-charge-path-checklist.md`, `tests/ui/delivery/delivery-manager-shortage-review-charge-path-fixture.ts`, and `tests/ui/delivery/delivery-manager-shortage-review-charge-path.spec.ts` | `keep` | `scenario-family-ready` | extend delivery coverage with rider acceptance, info-only or waive decisions, and final cashier settlement gating after the first manager shortage decision lane is stable |
