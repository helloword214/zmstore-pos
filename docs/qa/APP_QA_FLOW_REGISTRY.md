...
| `auth-role-routing` | `foundation` | auth | manager, rider, cashier, admin | land on role home after authenticated entry | `docs/guide/CANONICAL_IDENTITY_ACCESS_FLOW.md` | `tests/ui/*.golden-reference.spec.ts`, auth setup projects in `playwright.config.ts` | `rewrite` | `mapped` | create scenario coverage for post-login route routing and protected-route access per role |
...
| `cashier-shift-open-close` | `P0` | cashier | cashier | open shift, operate inside shift, then close or recount | `docs/guide/CANONICAL_CASHIER_SHIFT_VARIANCE_FLOW.md` | `tests/ui/cashier.golden-reference.spec.ts` route and screenshot shell checks | `rewrite` | `mapped` | replace shell checks with one cashier shift scenario family that covers real state transitions |
