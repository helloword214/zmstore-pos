...
| `delivery-cashier-order-remit-posting-happy-path` | `P0` | delivery | cashier | post the exact cash remit for a deterministic closed-run delivery order and return to the run remit hub | `docs/guide/CANONICAL_DELIVERY_CASH_AR_FLOW.md`, `docs/guide/RIDER_SHORTAGE_WORKFLOW.md` | canonical coverage is now `scripts/qa/delivery/delivery-cashier-order-remit-posting-happy-path-setup.ts`, `scripts/qa/delivery/delivery-cashier-order-remit-posting-happy-path-cleanup.ts`, `docs/qa/delivery/delivery-cashier-order-remit-posting-happy-path-checklist.md`, `tests/ui/delivery/delivery-cashier-order-remit-posting-happy-path-fixture.ts`, and `tests/ui/delivery/delivery-cashier-order-remit-posting-happy-path.spec.ts` | `keep` | `scenario-family-ready` | extend delivery coverage with shortage or partial-cash posting, manager shortage review, and rider-acceptance follow-through after the normal cashier remit path is stable |
