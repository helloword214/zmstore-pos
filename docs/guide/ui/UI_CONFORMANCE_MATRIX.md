...
| --- | --- | --- | --- | --- | --- | --- |
| `app/routes/store._index.tsx` | Manager dashboard | ALIGNED | REMIT | control-tower layout now uses a compact priority strip, a single dispatch workbench, quieter signal cards, short verb-first actions, a quieter route-transition handoff, and a planner-aligned workforce snapshot (`Duty today`, `Not checked`, `Checked`, `Absent`, `Replacement`, `Off today`, `Actual strength`) keyed to worker+day attendance facts so it matches the attendance-review board instead of stale schedule-link presence | manager actions are scan-first and inbox-first; keep specialized lanes linked from their hub pages instead of promoting them into the top dashboard queue | P2 |
| `app/routes/admin._index.tsx` | Admin dashboard | ALIGNED | REMIT | explicit `/admin` launchpad layout replaces the root-owned dashboard with concise start-here rows, primary create tiles, and calmer support/reference panels | admin lane is now action-first; keep lower-frequency support workflows out of the primary launch zone | P2 |
...
| `app/routes/cashier._index.tsx` | Cashier dashboard | ALIGNED | REMIT | focus-state layout now anchors the page on shift console, uses compact shift date labels, keeps attendance as a single date-accurate today-status workforce signal, and moves payroll/charge content into a quieter reference panel while keeping typed charge and shift aggregation intact | shift-first hierarchy is clear; monitor future reference density as payroll metadata grows | P2 |
...
| `app/routes/store.dispatch.tsx` | Dispatch queue | ALIGNED | REMIT | dispatch queue now keeps `Create Empty Run` as a quiet side-entry, moves selection and create controls into the inbox itself, and separates `New` versus `Failed` review through a simple top switch while preserving typed failed-delivery review and order-eligibility logic | keep failed-order detail blocks quiet and inbox-owned as exception volume grows | P2 |
| `app/routes/runs._index.tsx` | Runs index | ALIGNED | REMIT | runs list now defaults to an actionable inbox, moves terminal runs into an explicit history mode, keeps status-aware next-action labels with mobile action cards ahead of the preserved desktop table, progressively reveals older rows through a low-noise load-more cursor, and drops the duplicate create-run CTA so dispatch stays the clear creation lane | keep inbox/history boundaries and action labels synced as new run states or recap requirements are added | P2 |
...
| `app/routes/runs.$id.dispatch.tsx` | Run staging | ALIGNED | REMIT | staging now keeps assignment first, combines linked orders plus extra loadout into one load-plan workbench, and exposes only the dispatch action in editable mode while preserving typed loadout handling and fulfillment-status fallback logic | keep blocker copy concise if more stock evidence is added later | P2 |
| `app/routes/runs.$id.summary.tsx` | Run summary | ALIGNED | REMIT | SoT non-dashboard header + alerts + status badges + stock table primitives are applied, summary route-transition loading now uses a simpler shell-level handoff with route-aware labeling, and summary loader parsing avoids route-level `any` bypass (`riderCheckinSnapshot`, `loadoutSnapshot`, clearance case mapping) | keep recap helper copy concise as totals evolve | P2 |
| `app/routes/runs.$id.rider-checkin.tsx` | Rider check-in + CSS | ALIGNED | CHECKIN | SoT non-dashboard header + action-area loading states are applied, rider-checkin route-transition loading now uses a simpler shell-level handoff with route-aware labeling, and rider-checkin loader/action payload handling avoids route-level `any` bypass for snapshot/case parsing and submit-clearance guards | loading and lock states are clearer; reduce repeated hints in dense rows | P2 |
...
| `app/routes/ar._index.tsx` | Customer balances index | ALIGNED | REMIT | collection inbox now uses customer-facing balance language, leads with a compact open-balance summary strip, keeps search as a tighter toolbar, labels row actions as `Collect Payment`, and pages grouped customer balances server-side for larger ledgers | keep past-due emphasis compact if balance metadata expands | P2 |
| `app/routes/ar.customers.$id.tsx` | Customer balance | ALIGNED | REMIT | route now leads with a customer-facing open-balance hero, makes record-payment the dominant workbench, renames visible A/R jargon into balance/payment language, and moves statement/history/system refs into quieter secondary sections while preserving payment-post behavior | keep payment history collapsed unless collection review needs it open by default | P2 |
| `app/routes/ar.customers.$id_.statement.tsx` | Customer statement | ALIGNED | REMIT | statement route now uses the SoT non-dashboard header, compact date filter shell, a closing-balance-first summary, SoT table primitives, and customer-facing balance/payment row language while preserving statement math and print behavior | validate print layout after future statement columns are added | P2 |
| `app/routes/cashier.shift.tsx` | Cashier shift console | ALIGNED | REMIT | console/workspace pass now presents the open shift as a simple drawer-control home, moves cash movement and end-shift count into focused panels, and keeps drawer/payments/totals behind one details disclosure while preserving typed shift transaction and submit logic | revisit modal focus trapping if this route becomes keyboard-heavy in production QA | P2 |
| `app/routes/cashier.shift-history.tsx` | Cashier shift history | ALIGNED | REMIT | SoT non-dashboard header, compact filter card, and SoT table primitives now frame drawer movement separately from rider variance while preserving typed status filters and cash-count parsing | keep drawer and variance metadata compressed as more audit fields accumulate | P2 |
...
| `app/routes/store.workforce.schedule-templates.tsx` | Named staffing pattern library | ALIGNED | REMIT | the manager-facing route now stays a pure library-first named staffing pattern page, opens create/edit in a focused modal weekly editor, and leaves actual worker/date application to the planner instead of mixing assignment work into the library | keep the pattern modal calm and keep the library focused on reusable weekly shapes only | P2 |
| `app/routes/store.workforce.schedule-planner.tsx` | Workforce planner board | ALIGNED | REMIT | planner now keeps the primary flow on `window -> board -> publish`, uses the worker row as the only weekly `Apply pattern` lane for the visible window, replaces only matching `DRAFT` rows when a pattern is applied, leaves `PUBLISHED` rows under direct manual control, strips non-essential worker-row meta, removes repeated pattern-name noise from the board cells, and keeps the worker-date sheet focused on one-day preset/custom/manual edits | keep worker-row pattern entry calm and keep cell sheets strict to one-date exceptions only | P2 |
| `app/routes/store.workforce.suspension-records.tsx` | Workforce suspension records | ALIGNED | REMIT | suspension records now lead with a compact worker selector and a workbench-first suspension lane, keep the create form inline as the stable default, expose active suspension management above it, open edits in a dedicated modal, demote upcoming work into a quieter side reference, and move the audit history below the work surface so the main task stays obvious | keep create inline, edit modal-only, and avoid letting the history table reclaim the first viewport | P2 |
| `app/routes/store.workforce.attendance-review.tsx` | Workforce attendance review | ALIGNED | REMIT | attendance review now keeps one anomaly-friendly board in view, uses planner-safe date normalization so same-day `Off day` changes line up with attendance review, and opens a larger result-first modal with a flatter sheet layout, inline late toggle, and secondary-only work-context override | keep the board compact and let the modal reveal only the fields that matter for the chosen result | P2 |
