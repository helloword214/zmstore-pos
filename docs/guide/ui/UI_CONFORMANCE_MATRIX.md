...
| `app/routes/customers._index.tsx` | Admin customer list | ALIGNED | REMIT | SoT non-dashboard header + action bar + card/list shell are now applied with `ctx=admin` propagation; AR shortcut remains removed from admin lane | keep list row metadata compact as customer volume grows | P2 |
| `app/routes/customers.new.tsx` | Admin customer create | ALIGNED | REMIT | SoT non-dashboard header + card/action/form primitives are now applied with admin-context back/redirect mapping | maintain compact metadata phrasing in address rows as fields grow | P2 |
