...
| `app/routes/creation.riders.tsx` | Admin rider creation | PARTIAL | REMIT | SoT non-dashboard header + SoT card/button/form controls are applied, rider filters/forms now use shared SoT dropdown/select controls, and rider loader/action typing now avoids route-level `any` bypass in where-filter and error handling; row edit grid is still dense on narrow layouts | tighten row-column collapse behavior on smaller screens | P2 |
...
| `app/routes/creation.provinces.tsx` | Admin province creation | PARTIAL | REMIT | SoT non-dashboard header + SoT input/button controls are applied, and province loader/action typing now avoids route-level `any` bypass in filter and error paths; inline rename/code editing alignment still needs polish | align inline edit controls for stable row height | P2 |
...
| `app/routes/cashier._index.tsx` | Cashier dashboard | PARTIAL | REMIT | control sizing mostly aligned, and cashier dashboard loader now avoids route-level `any` bypass with enum-safe charge filters and typed outstanding-balance aggregation | maintain one CTA emphasis level across all action cards | P2 |
...
| `app/routes/rider._index.tsx` | Rider dashboard | PARTIAL | CHECKIN | control sizing mostly aligned, and rider dashboard loader now avoids route-level `any` bypass with enum-safe rider-role gating and typed open-charge payment aggregation | keep seller/rider panel emphasis balanced as cards evolve | P2 |
...
| `app/routes/store.clearance.tsx` | Clearance inbox | ALIGNED | CHECKIN | SoT non-dashboard header + card/table/status badge patterns are applied, and clearance inbox loader typing now avoids route-level `any` bypass for case-status filtering and walk-in/delivery mapping | no major drift; monitor tab-filter clarity only | P3 |
...
| `app/routes/cashier.delivery._index.tsx` | Cashier run list | PARTIAL | REMIT | SoT header + table/status badges are applied, and remit list loader paths now avoid route-level `any` bypass for run-order/cash-map parsing and lock checks | state labels can be clearer at first glance | P2 |
...
| `app/routes/ar._index.tsx` | AR index | PARTIAL | REMIT | SoT header + form field + table/button are applied, and route typing now avoids route-level `any` bypass | visual priority still spread across list metadata | P2 |
...
| `app/routes/cashier.shift-history.tsx` | Cashier shift history | PARTIAL | REMIT | SoT non-dashboard shell and shared dropdown filters are applied (`status`, `cashier`), and shift-history loader typing now avoids route-level `any` bypass in status filters/cash-count parsing | table density and action hierarchy still need final polish on compact screens | P2 |
...
| `app/routes/cashier.charges.tsx` | Cashier charge acknowledgment | PARTIAL | REMIT | SoT header + card/table/status primitives are applied, and charge loader/action paths now avoid route-level `any` bypass with enum-safe variance filters/status close writes plus typed denomination parsing | action flow is clearer, but note/ack blocks still compete for attention | P2 |
| `app/routes/store.cashier-ar.tsx` | Cashier AR tagging | PARTIAL | REMIT | SoT header + card/table/alert/button primitives are applied, and cashier-AR loader typing now avoids route-level `any` bypass in cashier identity mapping | tagging workflow is more consistent, but row metadata can still be trimmed | P2 |
...
| `app/routes/store.rider-variances.tsx` | Rider variance manager review | PARTIAL | REMIT | SoT header + card/table/status/button primitives are applied, and rider-variance loader/action paths now avoid route-level `any` bypass with enum-safe tab filters/decision transitions plus typed manager actor mapping | manager decision flow is clearer, but awaiting/history details still need compression | P2 |
| `app/routes/rider.variances.tsx` | Rider pending acceptance list | PARTIAL | CHECKIN | SoT header + card/table/button/status primitives are applied, and rider-pending loader typing now avoids route-level `any` bypass with typed session actor and enum-safe variance filters | acceptance intent is clearer, with minor metadata tightening still needed | P2 |
