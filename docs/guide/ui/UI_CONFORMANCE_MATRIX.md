...
| `app/routes/pad-order._index.tsx` | Order pad workspace | PARTIAL | N/A | SoT non-dashboard/page headers, card shell, and SoT input/alert patterns are now applied while preserving existing cart/create behavior and typed client guards | final drift remains in high-density product/cart controls on mobile sheet and inline quantity editors | P2 |
