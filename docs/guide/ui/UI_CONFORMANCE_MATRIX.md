...
| `app/routes/runs._index.tsx` | Runs index | ALIGNED | REMIT | runs list now shows clearer next-step labels, calmer header framing, and tighter row rhythm while preserving typed run status/filter handling | keep next-step labels synced with future run states | P2 |
