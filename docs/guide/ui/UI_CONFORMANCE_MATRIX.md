...
| `app/routes/runs._index.tsx` | Runs index | ALIGNED | REMIT | runs list now defaults to an actionable inbox, moves terminal runs into an explicit history mode, keeps status-aware next-action labels with mobile action cards ahead of the preserved desktop table, and progressively reveals older rows through a low-noise load-more cursor | keep inbox/history boundaries and action labels synced as new run states or recap requirements are added | P2 |
