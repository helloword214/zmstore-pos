...
| `app/routes/creation.areas.tsx` | Admin area hierarchy creation | ALIGNED | REMIT | compact hierarchy UX is applied with sticky navigator, single active workspace tab, and internal scroll tables while preserving strict parent-child validation | consider server-side pagination if area records become very large | P2 |
