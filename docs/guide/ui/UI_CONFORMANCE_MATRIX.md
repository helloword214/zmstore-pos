...
| `app/routes/products._index.tsx` | Product list (split baseline) | PARTIAL | REMIT | list shell now uses a tighter results summary, calmer core filters, and clearer target/indication filter summaries while preserving existing filter/action behavior, shared SoT dropdown controls, and typed fetcher/date handling | the list is cleaner now, but the full filter surface is still inherently dense and still needs one final mobile fit pass | P2 |
...
| `app/routes/creation.opening-ar-batches.tsx` | Opening balance batch encode | PARTIAL | N/A | SoT batch encode flow now includes explicit inline submit feedback, and admin opening-batch server paths avoid route-level `any` bypass for batch grouping and case/claim create payload writes | submit-state feedback is clearer; keep CSV/itemized composer hints concise as row volume grows | P2 |
...
| `app/routes/cashier.pos._index.tsx` | Cashier queue (walk-in) | PARTIAL | REMIT | SoT header + action bar + alert + table/status are applied, and queue action typing now avoids route-level `any` bypass in delete-slip error handling; row action controls still use route-local confirmation flows | keep action/control rhythm consistent with cashier remit list views | P2 |
...
| `app/routes/runs._index.tsx` | Runs index | PARTIAL | REMIT | runs list now adds a compact summary strip, denser run/rider rows, and more direct open-run actions while preserving typed run status/filter handling | one final pass is still needed to align column rhythm with the tighter dispatch list pattern | P2 |
...
| `app/routes/runs.$id.dispatch.tsx` | Run staging | PARTIAL | REMIT | staging now uses a clearer summary strip, calmer linked-order framing, and a more direct action workbench while preserving typed loadout handling and fulfillment-status fallback logic | loadout rows are cleaner, with one final compact/mobile fit pass still needed | P2 |
...
| `app/routes/store.clearance-opening-batches.tsx` | Opening balance clearance batch lane | PARTIAL | CHECKIN | opening-batch review now uses a compact summary strip, calmer batch list rows, and a more obvious exception-review workbench while preserving enum-safe decision/case status writes | bulk row density is improved, with one final compact/mobile fit pass still needed | P2 |
...
| `app/routes/cashier.delivery._index.tsx` | Cashier run list | PARTIAL | REMIT | cashier remit list now uses a compact remit summary strip, clearer lock-state rows, and more direct open/remit actions while preserving typed run-order cash-map parsing and lock checks | keep mobile table fit monitored as more remit-state metadata is added | P2 |
...
| `app/routes/cashier.shift-history.tsx` | Cashier shift history | PARTIAL | REMIT | history view now uses a compact summary strip, quieter filter shell, and denser drawer-activity columns while preserving typed status filters and cash-count parsing | compact/mobile table fit still needs a final pass as more drawer metadata accumulates | P2 |
...
| `app/routes/store.cashier-variances.tsx` | Cashier variance decision | PARTIAL | REMIT | decision table now uses shorter queue framing, clearer shift/status summaries, and a calmer evidence disclosure while preserving typed variance filters and denomination snapshots | keep evidence disclosure compact if more close-audit detail is added later | P2 |
| `app/routes/cashier.charges.tsx` | Cashier charge acknowledgment | PARTIAL | REMIT | charge review now separates note updates from the acknowledge-close action area while keeping enum-safe variance filters/status writes and typed denomination parsing intact | nested action blocks still need a final compact/mobile fit pass | P2 |
| `app/routes/store.cashier-ar.tsx` | Cashier AR tagging | PARTIAL | REMIT | tagging lane now adds a compact summary strip, shorter status metadata, and a cleaner payroll-plan action cell while preserving typed cashier identity mapping | monitor row density if more collection-plan variants are introduced | P2 |
| `app/routes/store.payroll.tsx` | Payroll settlement | PARTIAL | REMIT | payroll workbench now uses shorter blocker messaging, quieter metric cards, and a clearer deduction-review summary while keeping enum-safe charge/variance status updates intact | employee-side detail panels remain dense and still need a final responsive-fit pass | P2 |
| `app/routes/store.rider-variances.tsx` | Rider variance manager review | PARTIAL | REMIT | manager review queue now uses shorter tab framing, clearer shift/status summaries, and a calmer review/evidence split while preserving enum-safe tab filters, decision transitions, and typed manager actor mapping | open-row decision controls still need a final compact/mobile fit pass | P2 |
| `app/routes/rider.variances.tsx` | Rider pending acceptance list | PARTIAL | CHECKIN | rider pending list now adds a compact shortage summary and denser row summaries while keeping typed session actor and enum-safe variance filters intact | keep row density monitored on smaller screens as note length varies | P2 |
| `app/routes/rider.variance.$id.tsx` | Rider variance acceptance detail | PARTIAL | CHECKIN | acceptance detail now separates summary, evidence, and action areas while preserving typed rider actor mapping and enum-safe acceptance transitions | one final mobile spacing pass is still needed around the stacked evidence/action cards | P2 |
| `app/routes/store.rider-charges.tsx` | Rider charge tracking/tagging | PARTIAL | REMIT | rider charge tagging now uses a compact summary strip, shorter row metadata, and a cleaner payroll-plan action cell while preserving current charge-tagging behavior | review compact/mobile fit if more collection-plan variants are added later | P2 |
