...
| Dispatch queue | Manager | `app/routes/store.dispatch.tsx` | Select delivery orders, review failed-delivery returns, and start create/assign run from the dispatch lane | Order eligibility plus failed-delivery redispatch/cancel authority; no AR authority |
