...
This hub separates two different automation concerns:

1. UI consistency monitoring (`ui:cycle`)
2. Deterministic business-flow setup foundation (`automation:flow:setup` / `automation:flow:cleanup`)

## 2. Document Map

1. `docs/automation/runbooks/INTENT_ROUTER.md`
2. `docs/automation/runbooks/UI_CYCLE_RUNBOOK.md`
3. `docs/automation/architecture/BUSINESS_FLOW_ENGINE.md`
...
2. If the request is deterministic setup, delivery flow provisioning, or context preparation for delivery QA, use `BUSINESS_FLOW_ENGINE.md`.
...
2. Retired `automation:flow:smoke` should not be used.
3. Business-flow context comes from generated flow setup output (`FLOW_CONTEXT_FILE`).
...
Deterministic business-flow foundation:
...
3. Business-flow setup docs trail: `docs/automation/business-flow/runs/<timestamp>/`
