...
import {
  resolveEmployeeOnboardingCreateHappyPathScenarioContext,
  resetEmployeeOnboardingCreateHappyPathState,
} from "../../../scripts/qa/employee/employee-onboarding-create-happy-path-setup";

const DEFAULT_BASE_URL = "http://127.0.0.1:4173";

export const EMPLOYEE_ONBOARDING_CREATE_HAPPY_PATH_ENABLE_ENV =
  "QA_EMPLOYEE_ONBOARDING_CREATE_HAPPY_PATH_ENABLE";

export type EmployeeOnboardingCreateHappyPathScenario =
  Awaited<ReturnType<typeof resolveEmployeeOnboardingCreateHappyPathScenarioContext>>;
...
function parseCookiePair(setCookieHeader: string) {
  ...
  if (separatorIndex <= 0) {
    throw new Error(
      "Invalid auth cookie returned while creating the employee onboarding QA session.",
    );
  }
...
export function isEmployeeOnboardingCreateHappyPathEnabled() {
  return process.env[EMPLOYEE_ONBOARDING_CREATE_HAPPY_PATH_ENABLE_ENV] === "1";
}

export async function resolveEmployeeOnboardingCreateHappyPathContext() {
  return resolveEmployeeOnboardingCreateHappyPathScenarioContext();
}

export async function resetEmployeeOnboardingCreateHappyPathQaState() {
  return resetEmployeeOnboardingCreateHappyPathState();
}

export async function bootstrapEmployeeOnboardingCreateHappyPathSession(
  ...
  if (!setCookieHeader) {
    throw new Error(
      "Employee onboarding QA session bootstrap did not return a session cookie.",
    );
  }
...
export async function openEmployeeOnboardingCreateHappyPathCreatePage(page: Page) {
  const scenario = await resolveEmployeeOnboardingCreateHappyPathContext();
...
export async function openEmployeeOnboardingCreateHappyPathDirectoryPage(page: Page) {
  const scenario = await resolveEmployeeOnboardingCreateHappyPathContext();
...
export async function selectEmployeeOnboardingCreateHappyPathOption(
  page: Page,
  label: string,
  optionText: string,
) {
...
export function findEmployeeOnboardingCreateHappyPathDirectoryRow(page: Page, email: string) {
  return page.locator("tr").filter({ hasText: email }).first();
}

export async function resolveEmployeeOnboardingCreateHappyPathAccountState() {
  const scenario = await resolveEmployeeOnboardingCreateHappyPathContext();
...
export async function expectEmployeeOnboardingCreateHappyPathSuccessAlert(page: Page) {
  await expect(
...
export function expectEmployeeOnboardingCreateHappyPathDbState(accountState: Awaited<
  ReturnType<typeof resolveEmployeeOnboardingCreateHappyPathAccountState>
>) {
  ...
  expect(accountState?.roleAuditEvent?.beforeRole).toBe(UserRole.CASHIER);
  expect(accountState?.roleAuditEvent?.afterRole).toBe(UserRole.CASHIER);
