...
const AUTH_DIR = path.resolve(
  "test-results/ui/auth/delivery-manager-shortage-waive-info-only-path",
);
...
type ManagerDecisionResolution = "INFO_ONLY" | "WAIVE";

export type DeliveryManagerShortageWaiveInfoOnlyPathScenarioContext =
  DeliveryCashierOrderRemitShortagePathScenario & {
    infoOnlyDecisionNote: string;
    waiveDecisionNote: string;
...
function parseCookiePair(setCookieHeader: string) {
  ...
  if (separatorIndex <= 0) {
    throw new Error(
      "Invalid auth cookie returned while creating the delivery manager-shortage alternate-decision QA session.",
    );
  }
...
function buildDecisionNote(
  runCode: string,
  resolution: ManagerDecisionResolution,
) {
  if (resolution === "INFO_ONLY") {
    return `QA manager info-only decision for ${runCode}`;
  }
  return `QA manager waive decision for ${runCode}`;
}

export function resolveDeliveryManagerShortageWaiveInfoOnlyPathManagerEmail() {
  return normalizeEmail(
    process.env.QA_DELIVERY_MANAGER_SHORTAGE_WAIVE_INFO_ONLY_PATH_MANAGER_EMAIL ??
      process.env.UI_MANAGER_EMAIL ??
      DEFAULT_MANAGER_EMAIL,
  );
}

export function resolveDeliveryManagerShortageWaiveInfoOnlyPathManagerStateFilePath() {
...
async function resolveScenarioManager(email: string) {
  ...
  if (!user || !user.active || user.role !== UserRole.STORE_MANAGER) {
    throw new Error(
      `Delivery manager-shortage alternate-decision path requires an active STORE_MANAGER account: ${email}`,
    );
  }
...
async function seedOpenRiderShortageVariance(
  ...
  if (!order) {
    throw new Error(
      `Missing delivery order #${scenario.remitOrder.id} for manager alternate-decision setup.`,
    );
  }
  if (!taggedShift) {
    throw new Error(
      "Missing tagged open cashier shift for delivery manager alternate-decision setup.",
    );
  }
  if (!run?.riderId) {
    throw new Error(
      `Delivery run #${scenario.closedRun.id} is missing rider ownership for alternate-decision setup.`,
    );
  }
  if (order.runReceipts.length !== 1) {
    throw new Error(
      `Expected exactly one parent receipt for order #${order.id} during alternate-decision setup.`,
    );
  }
  if (order.payments.length > 0) {
    throw new Error(
      `Order #${order.id} already has payments before alternate-decision setup.`,
    );
  }
...
export async function deleteDeliveryManagerShortageWaiveInfoOnlyPathArtifacts(): Promise<DeleteSummary> {
  const deleted = await deleteDeliveryCashierOrderRemitShortagePathArtifacts();

  return {
    ...deleted,
    removedManagerStateFile: removeFileIfPresent(
      resolveDeliveryManagerShortageWaiveInfoOnlyPathManagerStateFilePath(),
    ),
  };
}

export async function resetDeliveryManagerShortageWaiveInfoOnlyPathState() {
  const deleted = await deleteDeliveryManagerShortageWaiveInfoOnlyPathArtifacts();
  await resetDeliveryCashierOrderRemitShortagePathState();
  const manager = await resolveScenarioManager(
    resolveDeliveryManagerShortageWaiveInfoOnlyPathManagerEmail(),
  );
...
export async function resolveDeliveryManagerShortageWaiveInfoOnlyPathScenarioContext(): Promise<DeliveryManagerShortageWaiveInfoOnlyPathScenarioContext> {
  ...
  if (!variance) {
    throw new Error(
      "Delivery manager-shortage alternate-decision path could not resolve the seeded rider variance.",
    );
  }

  return {
    ...shortageScenario,
    infoOnlyDecisionNote: buildDecisionNote(
      shortageScenario.closedRun.runCode,
      "INFO_ONLY",
    ),
    waiveDecisionNote: buildDecisionNote(
      shortageScenario.closedRun.runCode,
      "WAIVE",
    ),
    manager,
    ...
    historyRoute: "/store/rider-variances?tab=history",
...
async function main() {
  const { deleted, manager } =
    await resetDeliveryManagerShortageWaiveInfoOnlyPathState();
  const scenario =
    await resolveDeliveryManagerShortageWaiveInfoOnlyPathScenarioContext();

  console.log(
    [
      "Delivery manager shortage waive/info-only path setup is ready.",
      ...
      `History route: ${scenario.historyRoute}`,
      `Info-only note: ${scenario.infoOnlyDecisionNote}`,
      `Waive note: ${scenario.waiveDecisionNote}`,
      ...
      "3. Choose Info only or Waive, enter the matching printed note, and save.",
    ].join("\n"),
  );
}
    ...
    .catch((error) => {
      console.error(
        error instanceof Error
          ? error.message
          : "Unknown delivery manager-shortage alternate-decision setup error.",
      );
      throw error;
    })
    .finally(async () => {
