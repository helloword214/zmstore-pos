...
import { deleteWorkforceSchedulePlannerAssignmentGatingPathArtifacts } from "./workforce-schedule-planner-assignment-gating-path-setup";
...
async function main() {
  ...
  console.log(
    [
      "Workforce schedule planner assignment gating path cleanup complete.",
    ...
    .catch((error) => {
      console.error(
        error instanceof Error
          ? error.message
          : "Unknown workforce schedule planner assignment gating cleanup error.",
      );
      throw error;
    })
    .finally(async () => {
